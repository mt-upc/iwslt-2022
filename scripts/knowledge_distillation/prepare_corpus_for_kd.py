    ...
    with torch.no_grad():
        for sgm_ids, source_texts, target_texts in tqdm(iter(dataloader)):
            ...
            for i in range(len(logits)):
                ...
                logits_i, sorted_idx_i = torch.sort(
                    logits[i, 1:seq_len_i], dim=-1, descending=True
                )

                topk_probs_norm_before = F.softmax(logits_i, dim=-1)[:, : args.top_k]
                topk_probs_norm_after = F.softmax(logits_i[:, : args.top_k], dim=-1)
                topk_idx = sorted_idx_i[:, : args.top_k]
                top1_idx = topk_idx[:, 0]
                ...
                with open(args.path_to_output, "a") as f:
                    json.dump(
                        {
                            "id": sgm_ids[i],
                            "seq_len": seq_len_i.item() - 1,
                            "topk_idx": topk_idx.detach().cpu().tolist(),
                            "top1_tokens": tokenizer.convert_ids_to_tokens(
                                top1_idx.detach().cpu().tolist()
                            ),
                            "topk_probs_norm_before": topk_probs_norm_before.detach()
                            .cpu()
                            .tolist(),
                            "topk_probs_norm_after": topk_probs_norm_after.detach()
                            .cpu()
                            .tolist(),
                        },
                        f,
